 ...
 *
 * Runs inside GitHub Actions on logos-storage/roadmap (see .github/workflows/storage-weekly.yml).
...
const FORK = 'logos-storage/roadmap';
const BASE_BRANCH = 'v5';
...
export function compareUrl(facts, sourceUrl) {
  const r = facts.reported;
  return (
    `https://github.com/${UPSTREAM}/compare/${BASE_BRANCH}...logos-storage:roadmap:${encodeURIComponent(r.branch)}` +
...
async function main() {
  ...
  // Step 3 — guards, then generate the update file on a branch off upstream v5
